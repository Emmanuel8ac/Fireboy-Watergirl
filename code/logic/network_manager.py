...
class NetworkManager(QObject):
    ...
    @property
    def local_character(self):
        return self._local_character

    @property
    def remote_character(self):
    ...
    def create_room(self) -> str:
        ...
        self.status_changed.emit(
            f"Sala creada. Comparte esta IP con el otro jugador: {self._host}"
        )
    ...
    def join_room(self, code: str, timeout: float = 7.0) -> bool:
        ...
        found = self._discover_room(code, timeout)
        ...
        try:
            ...
            while time.time() < end_time:
                try:
                    ...
                    if (
                        message.get("type") == "fireboy_room"
                        and message.get("code") == code
                    ):
                        host = message.get("host") or address[0]
    ...
    def join_room_by_ip(self, host_ip: str) -> bool:
        saved_name = self._local_name
        self.disconnect()
        self._local_name = saved_name
        ...
        try:
            self._sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self._sock.settimeout(5)
            self._sock.connect((host_ip.strip(), DEFAULT_PORT))
            self._sock.settimeout(None)
            ...
            self.send_player_name()
            self.status_changed.emit(f"Conectado a {host_ip}.")
            return True

        except OSError as error:
            self._last_error = f"No pude conectarme a esa IP: {error}"
            self.status_changed.emit(self._last_error)
            self.disconnect()
            return False